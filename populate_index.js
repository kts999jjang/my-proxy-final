...
// --- 설정 ---
const INDEX_NAME = 'gcp-starter-gemini'; // 1단계에서 생성한 새 인덱스 이름
const BATCH_SIZE = 100; // 한 번에 Pinecone에 저장할 벡터 수
...
// --- 메인 실행 함수 ---
async function main() {
  console.log("데이터 준비 스크립트를 시작합니다...");
  ...
  for (const [themeName, themeData] of Object.entries(kInvestmentThemes)) {
    try {
      // 더 많은 기사를 가져오기 위해 max=100으로 설정
      const gnewsUrl = `https://gnews.io/api/v4/search?q=${encodeURIComponent(themeData.query)}&topic=business,technology&lang=en&max=100&apikey=${process.env.GNEWS_API_KEY}`;
  ...
  console.log(`총 ${uniqueArticles.length}개의 고유한 기사가 수집되었습니다.`);
  ...
  console.log("\n✨ 데이터 준비가 완료되었습니다! 이제 앱을 배포하고 실행하세요.");
}
